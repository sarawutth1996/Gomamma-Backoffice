...
import {
    Backdrop, Button, Box, Card, CardContent, TextField, Typography, Grid, IconButton, TableHead, TableSortLabel, TableFooter, TablePagination, FormLabel, FormControl,
    Table, TableBody, TableCell, TableContainer, TableRow, Paper, Dialog, DialogTitle, DialogContent, InputAdornment
} from "@material-ui/core";
import { Add, Close, Description, LocalOffer, Save, Subject, KeyboardArrowRight, KeyboardArrowLeft, LastPage, FirstPage, Search } from "@material-ui/icons";
import {
        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    <LocalOffer />
                    <Typography className={classes.font_header}>กำหนดส่วนลดโปรโมชั่น (Promotion)</Typography>
                </Box>
            </Box>
            <PageLine />
            <br />
        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    ...
                    <Typography className={classes.font_subheader}>&nbsp;ส่วนลดโปรโมชั่น</Typography>
                </Box>
                <PageLine />
            </Box>
...
const promotion = {
    ...
    promotion_desc: '',
    promotion_price: '',
    promotion_month: null,
    promotion_start: null,
    promotion_end: null,
    //------------------------
    promotion_file: '',
    promotion_base64: '',
    promotion_preview: '/image/demo_img.jpg',
}

export default function Promotion() {

    const url = config.API_URL + "models/Item/Item_promotion.php";
    ...
    async function SaveData() {
        ...
        if (res.status) {
            ...
            Swal.fire({
                ...
                state.push('/Home/ManagePromotion/' + vision);
            })
        }
    }
    ...
    const handleStartDate = (date) => {
        setForm({ ...Form, promotion_start: date });
    }

    const handleEndtDate = (date) => {
        setForm({ ...Form, promotion_end: date });
    }
                ...
                if (file) {
                    ...
                    reader.onload = () => {
                        if (reader.readyState === 2) {
                            setForm({ ...Form, promotion_file: file.name, promotion_preview: reader.result, promotion_base64: reader.result });
                        }
                    };
    ...
    const handlePreviewOpen = () => {
        (Form.promotion_preview !== "/image/demo_img.jpg") && setPreview(true);
    }
    ...
    const Submit = (event) => {
        ...
        if (output === datatable.length) {
            if (Form.promotion_preview !== "/image/demo_img.jpg") {
    ...
    return (
        <>
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={4} md={4} >
                            <Box className={classes.responsive_box}>
                                <img src={Form.promotion_preview}
                                    alt="Gomamma"
            ...
            {Preview &&
                <Lightbox
                    mainSrc={Form.promotion_preview}
