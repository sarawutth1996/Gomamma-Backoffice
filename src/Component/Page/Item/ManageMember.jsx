...
import { Add, Close, LocalOffer, Edit, KeyboardArrowRight, KeyboardArrowLeft, LastPage, FirstPage, Save, EventNote, Search } from "@material-ui/icons";
        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    <LocalOffer />
                    <Typography className={classes.font_header}>รายการส่วนลดสมาชิก</Typography>
                </Box>
            </Box>
            <PageLine />
            <br />
...
export default function ManageMember() {
    ...
    const headCells = [
        ...
        { id: 'gmm_member_name', label: 'รายการ', width: '35%', align: "left" },
        { id: 'gmm_member_start', label: 'เริ่ม', width: '15%', align: "center" },
        { id: 'gmm_member_end', label: 'สิ้นสุด', width: '15%', align: "center" },
        { id: 'gmm_member_active', label: 'Active', width: '15%', align: "center" },
    ...
    const handleActive = (row) => {
        Swal.fire({
            ...
            if (output.isConfirmed) {
                Update_status(row.gmm_member_active, row.gmm_member_id);
            }
        });
    }
    ...
    const filtered = datatable.filter((row) => {
        return row.gmm_member_name.toLowerCase().includes(search.toLowerCase())
    })
    ...
    const handleAdd = () => {
        state.push('/Home/Member/' + vision + '/create');
    }

    const handleEdit = (row) => {
        state.push('/Home/Member/' + vision + '/' + row.gmm_member_id);
    }
                                ...
                                : stableSort(filtered, getComparator(order, orderBy))).map((row, index) => (
                                    <StyledTableRow hover key={index}>
                                        ...
                                        <StyledTableCell width='15%' align="center">
                                            <Button
                                                title="Active"
                                                variant={row.gmm_member_active === 'ACTIVE' ? 'contained' : 'outlined'}
                                                color="primary"
                                                disabled={status}
