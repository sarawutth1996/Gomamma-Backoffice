...
import { Add, Close, Description, Edit, KeyboardArrowRight, KeyboardArrowLeft, LastPage, FirstPage, Save, EventNote, Search } from "@material-ui/icons";
        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    <Description />
                    <Typography className={classes.font_header}>รายการคูปอง</Typography>
                </Box>
            </Box>
            <PageLine />
            <br />
...
export default function ManagePackage() {
    ...
    const headCells = [
        ...
        { id: 'gmm_package_name', label: 'รายการ', width: '25%', align: "left" },
        { id: 'gmm_package_price', label: 'ราคา', width: '10%', align: "left" },
        { id: 'gmm_package_start', label: 'เริ่ม', width: '15%', align: "center" },
        { id: 'gmm_package_end', label: 'สิ้นสุด', width: '15%', align: "center" },
        { id: 'gmm_package_active', label: 'Active', width: '15%', align: "center" },
    ...
    const filtered = datatable.filter((row) => {
        return row.gmm_package_name.toLowerCase().includes(search.toLowerCase())
    })
    ...
    const handleAdd = () => {
        state.push('/Home/Package/' + vision + '/create');
    }

    const handleEdit = (row) => {
        state.push('/Home/Package/' + vision + '/' + row.gmm_package_id);
    }
    ...
    const handleDelete = (row) => {
        Swal.fire({
            ...
            if (output.isConfirmed) {
                delete_package(row.gmm_package_id);
            }
        });
    }
    ...
    const handleActive = (row) => {
        Swal.fire({
            ...
            if (output.isConfirmed) {
                Update_status(row.gmm_package_active, row.gmm_package_id);
            }
        });
    }
                                ...
                                : stableSort(filtered, getComparator(order, orderBy))).map((row, index) => (
                                    <StyledTableRow hover key={index}>
                                        ...
                                        <StyledTableCell width='20%' align="left">
                                            {row.gmm_package_name}
                                        </StyledTableCell>
                                        <StyledTableCell width='10%' align="left">
                                            {row.gmm_package_price.toLocaleString("en-US")}
                                        </StyledTableCell>
                                        <StyledTableCell width='15%' align="center">
                                            {row.gmm_package_start}
                                        </StyledTableCell>
                                        <StyledTableCell width='15%' align="center">
                                            {row.gmm_package_end}
                                        </StyledTableCell>
                                        <StyledTableCell width='15%' align="center">
                                            <Button
                                                title="Active"
                                                variant={row.gmm_package_active === 'ACTIVE' ? 'contained' : 'outlined'}
                                                color="primary"
                                                disabled={status}
