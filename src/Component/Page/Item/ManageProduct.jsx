...
import {
    Backdrop, Button, Box, TextField, Typography, Grid, IconButton, InputAdornment, Hidden, Switch,
    Table, TableSortLabel, TableHead, TableBody, TableCell, TableContainer, TableFooter, TablePagination, TableRow, Paper
} from "@material-ui/core";
import { Autocomplete } from "@material-ui/lab";
import { Add, Close, Description, Edit, Folder, Person, KeyboardArrowRight, KeyboardArrowLeft, LastPage, FirstPage, Save, EventNote, Search } from "@material-ui/icons";
        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    <Description />
                    <Typography className={classes.font_header}>รายการสินค้า</Typography>
                </Box>
            </Box>
            <PageLine />
            <br />
...
export default function ManageProduct() {
    ...
    const headCells = [
        ...
        { id: 'gmm_product_name', label: 'รายการสินค้า', width: '25%', align: "left" },
        { id: 'gmm_product_price', label: 'ราคา', width: '10%', align: "left" },
        { id: 'gmm_product_start', label: 'เริ่ม', width: '15%', align: "center" },
        { id: 'gmm_product_end', label: 'สิ้นสุด', width: '15%', align: "center" },
        { id: 'gmm_product_active', label: 'Active', width: '15%', align: "center" },
        { id: 'switch', label: 'display as package', width: '5%', align: "center" },
    ...
    const handleAdd = () => {
        state.push('/Home/Product/' + vision + '/create');
    }

    const handleEdit = (row) => {
        state.push('/Home/Product/' + vision + '/' + row.gmm_product_id);
    }
    ...
    const handleDelete = (row) => {
        Swal.fire({
            ...
            if (output.isConfirmed) {
                delete_user(row.gmm_product_id);
            }
        });
    }
    ...
    const handleActive = (row) => {
        Swal.fire({
            ...
            if (output.isConfirmed) {
                Update_status(row.gmm_product_active, row.gmm_product_id);
            }
        });
    }
    ...
    //                                         <Button
    //                                             title="Active"
    //                                             variant={row.gmm_product_active === 'ACTIVE' ? 'contained' : 'outlined'}
    //                                             color="primary"
    //                                             disabled={status}
