        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    <FileCopy />
                    <Typography className={classes.font_header}>รายงาน Timing & Distance</Typography>
                </Box>
            </Box>
            <PageLine />
            <br />
...
export default function Report_trip_calculator() {
    const url = config.API_URL + "models/Report/Report_trip_calc.php";
    ...
    async function LoadBooking(drop) {
        setBoolean(true);
        //--------------------------------
        const payload = JSON.stringify({
            key: "LoadBooking",
    ...
    const handleProducts = (ev, item) => {
        if (item) {
            setForm({ ...Form, Products: item, SortID: null, ToID: null });
            LoadBooking(item.gmm_booking_product_drop)
        } else {
            Clear();
        }
    }
                                    ...
                                    <Grid container spacing={2}>
                                        <Grid item xs={12} sm={6} >
                                            <Autocomplete
                                                ...
                                                options={Product}
                                                getOptionLabel={(value) => value.gmm_booking_product_name}
                                                getOptionSelected={(option, value) =>
                                                    option.gmm_booking_product_drop === value.gmm_booking_product_drop
                                                }
                                                value={Form.Products}
