...
import {
    Backdrop, Button, Box, Card, CardContent, FormControl, FormLabel, TextField, Typography, Grid, Hidden
        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    <FileCopy />
                    <Typography className={classes.font_header}>รายงานประเมินผล</Typography>
                </Box>
            </Box>
            <PageLine />
            <br />
...
export default function Report_assessment() {
    const url = config.API_URL + "models/Report/Report_assessment.php";
    const classes = useStyles();
    const { vision } = useParams();
    const [promise, setPromise] = useState(false)
    const [isLoading, setBoolean] = useState(false);
    const [Form, setForm] = useState(filterForm);
    const [SortType, setType] = useState([{ id: 'GTX', desc: 'Taxi' }, { id: 'GCG', desc: 'Caregiver' }, { id: 'GCM', desc: 'Customer' }]);
    ...
    async function LoadBooking(type) {
        setBoolean(true);
        //--------------------------------
    ...
    const handleSelectType = (ev, item) => {
        if (item) {
            setForm({ ...Form, Type: item, StartBooking: null, ToBooking: null });
            LoadBooking(item.id)
        } else {
            setForm({ ...Form, Type: null, StartBooking: null, ToBooking: null });
            setBooking([])
        }
    }


    const handleStartBooking = (ev, item) => {
        setForm({ ...Form, StartBooking: item });
    }

    const handleToBooking = (ev, item) => {
        setForm({ ...Form, ToBooking: item });
    }
                                    ...
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid item xs={12} sm={6} >
                                            <Autocomplete
                                                ...
                                                options={Booking}
                                                getOptionLabel={(value) => value.gmm_booking_rating_addr}
                                                getOptionSelected={(option, value) =>
                                                    option.gmm_booking_rating_addr === value.gmm_booking_rating_addr
                                                }
                                                value={Form.StartBooking}
                                                onChange={handleStartBooking}
                                                renderInput={(params) => (
                                                    <TextField
                                                        {...params}
                                        ...
                                        <Grid item xs={12} sm={6} >
                                            <Autocomplete
                                                ...
                                                value={Form.ToBooking}
                                                onChange={handleToBooking}
                                                renderInput={(params) => (
                                                    <TextField
                                                        {...params}
                                                        label="ถึง"
