        ...
        <>
            <Box pt={0} pb={0}>
                <Box className={classes.displayflex}>
                    <FileCopy />
                    <Typography className={classes.font_header}>รายงานโค้ดส่วนลด</Typography>
                </Box>
            </Box>
            <PageLine />
            <br />
...
export default function Report_coupon_master() {
    const url = config.API_URL + "models/Report/Report_coupon.php";
    const classes = useStyles();
    const { vision } = useParams();
    const [promise, setPromise] = useState(false)
    const [isLoading, setBoolean] = useState(false);
    const [Form, setForm] = useState(filterForm);
    const [SortType, SetType] = useState([{ status: 'CODE' }, { status: 'NEWMEMBER' }, { status: 'RUNNING' }]);
    ...
    const handleStartType = (ev, item) => {
        setForm({ ...Form, StartType: item });
    }

    const handleToType = (ev, item) => {
        setForm({ ...Form, ToType: item });
    }
